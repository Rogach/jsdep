...
files.forEach(function(file) {
  ...
  if (file.rjsPath != "utils/logger" && file.rjsPath != "main") {
    var requiresLog = false;
    recast.types.visit(file.ast, {
      visitIdentifier: function(path) {
        if (path.value.name == "log") {
          requiresLog = true;
          return false;
        } else {
          this.traverse(path);
    ...
    if (requiresLog) {
      file.dependencies.push("utils/logger");
      recast.types.visit(file.ast, {
        visitCallExpression: function(path) {
          if (definePredicate(path.value) &&
              n.FunctionExpression.check(path.value.arguments[1])
             ) {
            path.get("arguments", 1, "body", "body").unshift(
              b.variableDeclaration("var", [
                b.variableDeclarator(
                  b.identifier("log"),
                  b.callExpression(b.identifier("logger"), [b.literal(file.rjsPath)])
                )
              ])
            );
          }
...
files.forEach(function(file) {
  ...
  mkdirp(fsPath.dirname(fsPath.join(file.outputFile)));
